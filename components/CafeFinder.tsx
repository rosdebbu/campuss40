    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
      ...
      <div className="p-4">
        <div className="flex justify-between items-start">
            <div>
                <h3 className="text-xl font-bold text-stone-800">{cafe.name}</h3>
        ...
        <div className="flex items-center mt-2">
          ...
          <span className="text-stone-600 text-sm ml-2">{cafe.rating.toFixed(1)} ({cafe.reviewCount} reviews)</span>
        </div>
      </div>
    </div>
  );
      ...
      case 'loading':
        return (
          <div className="text-center py-12" role="status">
            <svg className="animate-spin h-10 w-10 text-amber-800 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="mt-4 text-stone-600 text-lg">Finding the best cafes near you...</p>
          </div>
        );
      case 'results':
      case 'saved':
        ...
        return (
          <div className="w-full">
            <div className="flex justify-between items-center mb-6 px-2">
                 ...
                 <button onClick={() => setView('idle')} className="text-amber-800 font-semibold hover:underline">
                    &larr; Back
                 </button>
            </div>
            {cafesToShow.length > 0 ? (
                ...
                <div className="text-center bg-stone-100 p-8 rounded-lg shadow-sm">
                    <h3 className="text-xl font-semibold text-stone-800">No Saved Cafes Yet</h3>
                    <p className="text-stone-500 mt-2">Click the bookmark icon on a cafe to save it here.</p>
        ...
        return (
            <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
                <button 
                    ...
                    className="px-8 py-4 bg-amber-800 text-white font-semibold rounded-full shadow-lg hover:bg-amber-900 transition-colors transform hover:scale-105 duration-300">
                    Find Cafes Near Me
                </button>
                <button 
                    onClick={() => setView('saved')}
                    className="px-8 py-4 bg-stone-700 text-white font-semibold rounded-full shadow-lg hover:bg-stone-800 transition-colors transform hover:scale-105 duration-300">
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
           <div className="flex justify-center items-center mb-4">
               <svg className="h-8 w-8 text-amber-800 mr-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
               ...
               <h2 className="text-3xl md:text-4xl font-extrabold text-stone-800">Cafe Curator</h2>
           </div>
           <p className="text-lg text-stone-500 max-w-2xl mx-auto">Discover and keep track of the best coffee spots around campus.</p>
        </div>
        {renderContent()}
      </div>
    </section>
